...
(function (global, document) {
  ...
  function isTransparentColor(color) {
    return !color || color === 'transparent' || (
      color.indexOf('rgba(') === 0 && /,\s*0\)$/.test(color)
    );
  }

  function readBackgroundColor(state) {
    var current = state.screen ? document.body : state.panel;
    ...
    var rootColor = getComputedStyle(document.documentElement).backgroundColor;
    return isTransparentColor(rootColor) ? '#ffffff' : rootColor;
  }

  function parseRgbColor(color) {
    var hex = color && color.match(/^#([0-9a-f]{3}|[0-9a-f]{6})$/i);
    if (hex) {
      var value = hex[1];
      if (value.length === 3) {
  ...
  function getOppositeColor(color) {
    var rgb = parseRgbColor(color) || { r: 255, g: 255, b: 255 };
  ...
  function play(panel, overrides) {
    ...
    if (
      panel.dataset.rcrRingColor === undefined &&
      (!overrides || overrides.ringColor === undefined)
    ) {
      var backgroundColor = readBackgroundColor({
        panel: panel,
        screen: config.screen
      }) || '#ffffff';
      config.ringColor = getOppositeColor(backgroundColor);
    }

    // 黒いCanvasの塗り色だけを、表示対象のCSS背景色に合わせます。
    // 円形のdestination-out処理やレイヤー構造は変更しません。
    if (panel.dataset.rcrCoverColor === undefined) {
      config.coverColor = readBackgroundColor({
        panel: panel,
        screen: config.screen
      }) || config.coverColor;
    }
